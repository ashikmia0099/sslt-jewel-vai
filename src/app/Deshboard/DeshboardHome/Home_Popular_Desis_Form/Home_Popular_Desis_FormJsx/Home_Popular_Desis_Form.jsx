...
import { postfetchMedicalDesies } from "@/redux/features/Home/popularMedicalDesiesSlice";
...
const handleFormSubmit = async (e) => {
    ...
    const Desis_Name = form.Desis_Name.value;
    const Description_Title = form.Description_Title.value;
    const Description = form.Description.value;
    const Choose_Image = form.Choose_Image.files[0];

    const formData = new FormData();

    formData.append("Desis_Name", Desis_Name);
    ...
    try {
        const resultAction = await dispatch(postfetchMedicalDesies(formData));

        if (postfetchMedicalDesies.fulfilled.match(resultAction)) {
            ...
            <form onSubmit={handleFormSubmit}>
                <div className=' gap-3'>
                    <div>
                        ...
                        <input type="text" name='Desis_Name' className="input w-full  text-lg" placeholder="Desis Name" required />
                    </div>
                </div>
                <div>
